...
class ImageFeatureExtractor(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        # 用于提取空间特征的3D卷积层
        self.spatial_3d_conv = nn.Conv3d(out_channels, out_channels, kernel_size=(depth, 3, 3), padding=(1,1,1))
        # 用于提取通道特征的3D卷积层（1x1x1卷积核）
        self.channel_3d_conv = nn.Conv3d(out_channels, out_channels, kernel_size=(depth, 1, 1), padding=(1,0,0))
    ...
    def forward(self, x):
        ...
        # 使用3D卷积提取空间特征
        spatial_features = self.spatial_3d_conv(combined_features)
        # 使用3D卷积提取通道特征
        channel_features = self.channel_3d_conv(combined_features)
        # to2D
        spatial_features = self.pool_layer1(spatial_features).squeeze(2)
        channel_features = self.pool_layer2(channel_features).squeeze(2)
