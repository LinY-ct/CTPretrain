...
from models.V3.network import ResNet34Mvt
...
class ImageFeatureExtractor(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.resnet34Mvt=ResNet34Mvt(in_channels,out_channels)
    ...
    def forward(self, x):
        ...
        feature_1 =self.resnet34Mvt(x)
        spatial_features, channel_features = feature_1,feature_1
...
class D3DIR_V4_Encoder(FBPTools):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        super(D3DIR_V4_Encoder, self).__init__()

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)
        self.decoder = decoder

        if self.decoder:
            # self.fre1 = FreModule(dim, num_heads=heads[2], bias=bias)
            self.fre2 = FreModule(dim * 2 ** 1, num_heads=heads[2], bias=bias)
            self.fre3 = FreModule(dim * 2 ** 2, num_heads=heads[2], bias=bias)
            self.fre4 = FreModule(dim * 2 ** 3, num_heads=heads[2], bias=bias)
    ...
    def forward(self, inp_img, noise_emb=None):
        ...
        # if self.decoder:
        #     inp_enc_level1 = self.fre1(inp_enc_level1)
        ...
        if self.decoder:
            inp_enc_level2 = self.fre2(inp_enc_level2)
        out_enc_level2 = self.encoder_level2(inp_enc_level2)

        inp_enc_level3 = self.down2_3(out_enc_level2)

        if self.decoder:
            inp_enc_level3 = self.fre3(inp_enc_level3)
        out_enc_level3 = self.encoder_level3(inp_enc_level3)

        inp_enc_level4 = self.down3_4(out_enc_level3)
        if self.decoder:
            inp_enc_level4 = self.fre4(inp_enc_level4)
