...
class dynamic_filter_channel(nn.Module):
    ...
    def forward(self, x):
        ...
        #print("low_filter shape:", low_filter.shape)
...
class PromptModule(nn.Module):
    def __init__(self, basic_dim=32, dim=32, input_resolution=128):
        ...
        self.simple_Fusion = nn.Conv2d(3 * dim, dim, kernel_size=1, stride=1)
        ...
        self.Dino_high = frequenctSpecificPromptGenetator(basic_dim, h, w, flag_highF=True)
        ...
        self.modulator_dino = highFrequencyPromptFusion(dim, basic_dim, win_size=8, num_heads=2, bias=False)
        ...
        self.conv = nn.Conv2d(1, basic_dim, kernel_size=1, stride=1)
    ...
    def unpatchify(self, x):
        ...
        return imgs
    def forward(self, low_part, out_high, x, dino_fre):
        b, c, h, w = x.shape

        dino_fre = self.dinov2_decoder(dino_fre)
        dino_fre = self.unpatchify(dino_fre)
        dino_fre = self.conv(dino_fre)

        dino_fre_h = self.Dino_high(dino_fre, h, w)
        y_h = self.FSPG_high(out_high, h, w)
        y_l = self.FSPG_low(low_part, h, w)

        y_h = self.modulator_hi(x, y_h)
        y_l = self.modulator_lo(x, y_l)
        y_dino = self.modulator_dino(x, dino_fre_h)

        x = self.simple_Fusion(torch.cat([y_h, y_l, y_dino], dim=1))
...
class FPro_v3(FBPTools):
    def __init__(self,
        ...
        super(FPro_v3, self).__init__()
        self.dinov2_model = dinov2()
        net_checkpoint = torch.load("/mnt/nas/wsy/Codes/NewCodes/demo4/Experiments/resume/aapm/ckpt2/dinov2/dinov2-net-best_val.pth"
                                    , map_location='cpu')
    ...
    def forward(self, inp_img):
        dino_LQ_Features = self.dinov2_model.dinov2_model.get_intermediate_layers(
            F.interpolate(inp_img, size=(224, 224),
                          mode='bilinear', align_corners=False), n=[0, 3, 7, 11], reshape=False)
        ...
        out_dec_level3 = self.prompt_d3(low_part, out_high, out_dec_level3, dino_LQ_Features[0]) + out_dec_level3

        inp_dec_level2 = self.up3_2(out_dec_level3)
        inp_dec_level2 = torch.cat([inp_dec_level2, out_enc_level2], 1)
        inp_dec_level2 = self.reduce_chan_level2(inp_dec_level2)
        out_dec_level2 = self.decoder_level2(inp_dec_level2)
        out_dec_level2 = self.prompt_d2(low_part, out_high, out_dec_level2,dino_LQ_Features[1]) + out_dec_level2

        inp_dec_level1 = self.up2_1(out_dec_level2)
        inp_dec_level1 = torch.cat([inp_dec_level1, out_enc_level1], 1)
        out_dec_level1 = self.decoder_level1(inp_dec_level1)
        out_dec_level1 = self.prompt_d1(low_part, out_high, out_dec_level1, dino_LQ_Features[2]) + out_dec_level1

        out_dec_level1 = self.refinement(out_dec_level1)
        out_dec_level1 = self.prompt_r(low_part, out_high, out_dec_level1, dino_LQ_Features[3]) + out_dec_level1
