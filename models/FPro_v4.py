...
class FPro_v4(FBPTools):
    def __init__(self,
        ...
        self.dino_patch_embed = OverlapPatchEmbed(1, dim)
        self.dino_splitFre = splitFrequencyModule(basic_dim=dim, dim=int(dim * 2 ** 2), input_resolution=32)
        self.dino_prompt_e1 = PromptModule(basic_dim=dim, dim=dim, input_resolution=256)
        self.dino_prompt_e2 = PromptModule(basic_dim=dim, dim=int(dim * 2 ** 1), input_resolution=128)
        self.dino_prompt_e3 = PromptModule(basic_dim=dim, dim=int(dim * 2 ** 2), input_resolution=64)
    ...
    def forward(self, inp_img):
        dino_output = self.dinov2_model(F.interpolate(inp_img, size=(224, 224),mode='bilinear', align_corners=False))
        dino_output = self.dinov2_decoder(dino_output)
        dino_output = self.dinov2_model.unpatchify(dino_output)
        dino_output = self.dino_patch_embed(dino_output)

        inp_enc_level1 = self.patch_embed(inp_img)
        out_enc_level1 = self.encoder_level1(inp_enc_level1)
        dino_low_part, dino_out_high = self.dino_splitFre(dino_output)
        # print("out_enc_level1 shape:", out_enc_level1.shape)
        out_enc_level1 = self.dino_prompt_e1(dino_low_part, dino_out_high, out_enc_level1) + out_enc_level1

        inp_enc_level2 = self.down1_2(out_enc_level1)
        out_enc_level2 = self.encoder_level2(inp_enc_level2)
        out_enc_level2 = self.dino_prompt_e2(dino_low_part, dino_out_high, out_enc_level2) + out_enc_level2

        inp_enc_level3 = self.down2_3(out_enc_level2)
        out_enc_level3 = self.encoder_level3(inp_enc_level3)
        out_enc_level3 = self.dino_prompt_e3(dino_low_part, dino_out_high, out_enc_level3) + out_enc_level3
