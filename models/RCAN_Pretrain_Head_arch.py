...
class ResidualBlock_noBN_modulation(nn.Module):
    ...
    def __init__(self, nf=64, degradation_embed_dim=512):
        ...
        self.modulation_layer = StyleGAN2_mod(out_channels=nf, modul_channels=degradation_embed_dim, kernel_size=3,
                                              noise=True)
...
class RCAB_modulation(nn.Module):
    def __init__(
        ...
        # print(degradation_embed_dim)
        self.modulation_layer = StyleGAN2_mod(out_channels=n_feat, modul_channels=degradation_embed_dim, kernel_size=3,
                                              noise=True)
...
class ResidualGroup_modulation(nn.Module):
    def __init__(self, conv, n_feat, kernel_size, reduction, act, res_scale, n_resblocks, degradation_embed_dim=512):
        ...
        self.modules_body1 = RCAB_modulation(
            conv, n_feat, kernel_size, reduction, bias=True, bn=False, act=nn.ReLU(True), res_scale=1,
            degradation_embed_dim=degradation_embed_dim)
...
class RCAN_Head(nn.Module):
    def __init__(self, conv=default_conv, in_c=3, out_c=3, n_feats=64, scale=1, require_modulation=False,
                 degradation_embed_dim=512):
...
if __name__ == '__main__':
    ...
    model_Head = MSRResNet_Head(in_c=180, out_c=3, nf=64, scale=1, require_modulation=True,
                                degradation_embed_dim=512).cuda()
