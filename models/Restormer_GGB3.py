...
class Restormer_GGB3(FBPTools):
    def __init__(self,
        ...
        super(Restormer_GGB3, self).__init__()

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)
        self.decoder = decoder

        if self.decoder:
            self.fre1 = GGB(dim * 2 ** 3, num_heads=heads[2], bias=bias)
            self.fre2 = GGB(dim * 2 ** 2, num_heads=heads[2], bias=bias)
            self.fre3 = GGB(dim * 2 ** 1, num_heads=heads[2], bias=bias)
