...
class Restormer_GGB3_Encoder_Decoder_v2(FBPTools):
    def __init__(self,
                 inp_channels=3,
                 out_channels=3,
                 dim=48,
        ...
        super(Restormer_GGB3_Encoder_Decoder_v2, self).__init__()

        self.patch_embed = OverlapPatchEmbed(inp_channels, dim)
        self.decoder = decoder

        if self.decoder:
            self.fre1 = GGB(dim, num_heads=heads[2], bias=bias)
            self.fre2 = GGB(dim * 2 ** 1, num_heads=heads[2], bias=bias)
            self.fre3 = GGB(dim * 2 ** 2, num_heads=heads[2], bias=bias)
            self.fre4 = GGB(dim * 2 ** 3, num_heads=heads[2], bias=bias)
            self.fre5 = GGB(dim * 2 ** 2, num_heads=heads[2], bias=bias)
            self.fre6 = GGB(dim * 2 ** 1, num_heads=heads[2], bias=bias)
    ...
    def forward(self, inp_img, noise_emb=None):
        ...
        if self.decoder:
            inp_enc_level1 = self.fre1(inp_img, inp_enc_level1)
        ...
        if self.decoder:
            inp_enc_level2 = self.fre2(inp_img, inp_enc_level2)
        ...
        if self.decoder:
            inp_enc_level3 = self.fre3(inp_img, inp_enc_level3)
        ...
        if self.decoder:
            inp_enc_level4 = self.fre4(inp_img, inp_enc_level4)
        ...
        if self.decoder:
            out_dec_level3 = self.fre5(inp_img, out_dec_level3)
        ...
        if self.decoder:
            out_dec_level2 = self.fre6(inp_img, out_dec_level2)
        ...
        # fft = torch.fft.fft2(out_dec_level1, norm='forward', dim=(-2, -1))
        mask = self.gen_gaussian_bandpass(self.center, self.width, shape=out_dec_level1.shape[2:])
        # fft_high = mask * out_dec_level1
        # high = torch.fft.ifft2(fft_high, norm='forward', dim=(-2, -1))
        # high = torch.abs(high)

        return out_dec_level1, mask
