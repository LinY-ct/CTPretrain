...
from models.V4_1.activation import mish
...
class ResNet34Mvt(nn.Module):
    def __init__(self,in_channels,out_channels,num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, attn_drop=0., 
        ...
        self.conv7 = nn.Sequential(nn.Conv3d(32, out_channels=out_channels, kernel_size=(1,1,1), stride=(1,1,1),padding=0),mish())
    ...
    def forward(self, x):
        ...
        f1 = f1 + x1
        f1 = self.pca_torch(f1,n_components=1)
    ...
    def pca_torch(self, x1, n_components=1):
        """
        PCA 实现：使用 PyTorch 进行更高效的降维
        """
        batch_size, channels, depth, height, width = x1.shape

        # 展平输入张量
        x1_flat = x1.view(batch_size * channels * height * width, depth)

        # 计算协方差矩阵
        x1_centered = x1_flat - torch.mean(x1_flat, dim=0)
        cov_matrix = torch.matmul(x1_centered.T, x1_centered) / x1_centered.size(0)
        ...
        # 降维
        x1_pca = torch.matmul(x1_centered, eigvecs[:, :n_components])

        # 恢复为原始形状
        x1_pca = x1_pca.view(batch_size, channels, n_components, height, width)
