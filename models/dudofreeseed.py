...
sys.path.append('..')
from models.freeseed import FreeNet, SeedNet
import numpy as np
...
class dudofreeseed(FBPTools):
    def __init__(
            self, img_size=256, num_full_views=720, source_distance=1075, det_count=672,
            ratio_ginout=0.5, mask_type=None, min_channels=64):
        super().__init__()
        wrapper_kwargs = dict(img_size=256, num_full_views=720, source_distance=1075, det_count=672)
        ...
        self.img_net = FreeNet(2, 1)

    def forward(self, sparse_sino, sparse_mu, sparse_sino_mask, global_skip=True, only_train_img_net=False):
        ...
        return pred_mu, sino, ril_mu  # 整个网络的输出，sino_net的输出， sino_net的输出转为CT-Image
