...
sys.path.append('..')
from models.modules.basic_layers import get_conv_layer, get_norm_layer, get_act_layer, get_conv_block,forward_channel_last
...
class FourierUnit(nn.Module):
    def __init__(
        ...
        if mask_type is not None:
            if mask_type == 'global':
                ...
                self.mask = nn.Parameter(torch.randn((1, ffc_in_channels, *fft_size), dtype=torch.float32) * 0.02,
                                         requires_grad=True)
                ...
                self.mask = None
                self.center = nn.Parameter(torch.tensor([0. for _ in range(ffc_in_channels)], dtype=torch.float32),
                                           requires_grad=True)
                self.width = nn.Parameter(torch.tensor([1. for _ in range(ffc_in_channels)], dtype=torch.float32),
                                          requires_grad=True)
    ...
    def forward(self, x):
        ...
        if self.spectral_pos_encoding:
            ...
            coords_vertical = torch.linspace(0, 1, height)[None, None, :, None].expand(batch, 1, height, width).to(
                ffted)
            coords_horizontal = torch.linspace(0, 1, width)[None, None, None, :].expand(batch, 1, height, width).to(
                ffted)
...
class FFCBlock(nn.Module):
    def __init__(
        ...
        self.norm_local = nn.Identity() if ratio_gout == 1 else get_norm_layer(norm_type,
...
class EncodingBlock(nn.Module):
    def __init__(
            self, in_channels, out_channels, kernel_size=3, pooling=True, norm_type='BATCH', act_type='LRELU',
            act_kwargs=None,
            use_ffc=False, ratio_gin=0, ratio_gout=0, enable_lfu=False, mask_type=None, fft_size=(256, 129)):
        super().__init__()
        self.pooling = pooling
        conv_kwargs = {'kernel_size': kernel_size, 'norm_type': norm_type, 'act_type': act_type,
                       'act_kwargs': act_kwargs, }
        fu_kwargs = {'ratio_gin': ratio_gin, 'ratio_gout': ratio_gout, 'enable_lfu': enable_lfu,
                     'mask_type': mask_type, 'fft_size': fft_size}
        if pooling:
            self.down = nn.MaxPool2d(2)
            self.conv = MultipleConv(in_channels, out_channels, use_ffc=use_ffc, num_convs=2, **conv_kwargs,
                                     **fu_kwargs)
        else:
            self.down = MultipleConv(in_channels, in_channels, kernel_size=3, stride=2, padding=1, norm_type=norm_type,
                                     act_type=act_type, act_kwargs=act_kwargs, use_ffc=use_ffc, num_convs=1,
                                     **fu_kwargs)
            self.conv = MultipleConv(in_channels, out_channels, use_ffc=use_ffc, num_convs=1, **conv_kwargs,
                                     **fu_kwargs)
...
class DecodingBlock(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, bilinear=True, norm_type='BATCH', act_type='RELU',
                 act_kwargs=None,
                 skip_mode=None):
        super().__init__()
        conv_kwargs = {'kernel_size': kernel_size, 'norm_type': norm_type, 'act_type': act_type,
        ...
        if bilinear:
            ...
            self.conv = MultipleConv(in_channels, in_channels // 2, use_ffc=False, num_convs=1, **conv_kwargs,
                                     **fu_kwargs)
            self.merge = MultipleConv(in_channels, out_channels, in_channels // 2, use_ffc=False, **conv_kwargs,
                                      **fu_kwargs)
        ...
        if skip_mode == 'conv':
            self.skip_block = MultipleConv(in_channels // 2, in_channels // 2, use_ffc=False, num_convs=1,
                                           **conv_kwargs, **fu_kwargs)
        elif skip_mode == 'fourier':
            self.skip_block = MultipleConv(in_channels // 2, in_channels // 2, use_ffc=True, num_convs=1, **conv_kwargs,
                                           **fu_kwargs)
        else:
...
class FreeNet(FBPTools):
    def __init__(
            self, in_channels=3, out_channels=3, factor=2, min_channels=32, max_channels=1024, num_in_conv=2, num_stages=5,
            norm_type='BATCH', act_type='RELU', act_kwargs=None, bilinear=True, skip_mode=None,
            use_fft=True, ratio_ginout=0.5, global_skip=False, mask_type_init=None, mask_type_down=None,
            fft_size=(256, 129),
           ):
        super().__init__()
        ...
        self.inc = nn.Sequential(
            MultipleConv(in_channels, feature_channels[0], num_convs=1, use_ffc=use_fft, **conv_kwargs,
                         **init_fu_kwargs),
            MultipleConv(feature_channels[0], feature_channels[0], num_convs=num_in_conv - 1, use_ffc=use_fft,
                         **conv_kwargs, **down_fu_kwargs),
        )
...
class SeedNet(FBPTools):
    def __init__(
            self, in_channels=3, out_channels=3, ngf=64, n_downsampling=1, n_blocks=3, norm_type='BATCH',
        ...
        latent_conv_kwargs = {'ratio_gin': ratio_gin, 'ratio_gout': ratio_gout, 'enable_lfu': enable_lfu,
                              'gated': gated} if latent_conv_kwargs is None else latent_conv_kwargs

        self.inc = nn.Sequential(
            nn.ReflectionPad2d(3),
            FFCBlock(in_channels, ngf, kernel_size=7, padding=0, norm_type=norm_type, act_type=act_type,
                     **init_conv_kwargs)
        )
        ...
        if out_ffc:
            self.outc.append(
                FFCResNetBlock(ngf, padding_mode=padding_mode, norm_type=norm_type, act_type=act_type, inline=True))
        self.outc += [
            nn.ReflectionPad2d(3),
