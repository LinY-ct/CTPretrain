...
class ResNet34Mvt(nn.Module):
    def __init__(self,initial_outputshape,num_class=21, dim=768, num_heads=12, mlp_ratio=4., qkv_bias=False, qk_scale=None, attn_drop=0., 
        ...
        self.conv1 = nn.Conv2d(128, 32, kernel_size=1, stride=1)
        self.conv2 = nn.Conv2d(256, 32, kernel_size=1, stride=1)

        self.conv3 = nn.Conv2d(32, 1, kernel_size=1, stride=1)
        self.outputshape = initial_outputshape
        self.upsample1 = nn.Upsample(size=(self.outputshape, self.outputshape), mode='bilinear', align_corners=False)
        self.upsample2 = nn.Upsample(size=(self.outputshape, self.outputshape), mode='bilinear', align_corners=False)
    ...
    def forward(self, x,outputshape=None):
        if outputshape is not None:
            # 直接使用传入的 outputshape 创建新的上采样层
            upsample1 = nn.Upsample(size=(outputshape, outputshape), mode='bilinear', align_corners=False)
            upsample2 = nn.Upsample(size=(outputshape, outputshape), mode='bilinear', align_corners=False)
        else:
            # 否则使用初始化时的 self.outputshape
        ...
        # print("self.backbone.layer4 input shape:", x.shape)
        ...
        f1_upsampled = upsample1(f1)
        f2_upsampled = upsample2(f2)

        # print('f1_upsampled:',f1_upsampled.shape)
        # print('f1_upsampled:',f1_upsampled.shape)
        fm = self.conv3(f1_upsampled+f2_upsampled)
