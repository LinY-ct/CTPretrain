...
def main(opt):
    ...
    elif opt.network  == 'Restormer_GGB3_Encoder':
    ...
    if opt.trainer_mode == 'train':
        trainer = None
        if opt.network in ['Restormer_GGB3_Encoder_Decoder_v2', 'Restormer_GGB3_Encoder_GGB3CLv2']:
            print('load trainer : ' + 'piglet_trainer_GGB3CL_v2')
            trainer = load_class('trainers', 'piglet_trainer_GGB3CL_v2', opt=opt, net=net)
            trainer.fit()
            ...
            print('load trainer : ' + 'piglet_trainer_AMIR')
            trainer = load_class('trainers', 'piglet_trainer_AMIR', opt=opt, net=net)
            trainer.fit()
        elif 'PIPNet_Restormer_onskip_inter' == opt.network :
            print('load trainer : ' + 'trainer_PIP')
            trainer = load_class('trainers', 'trainer_PIP', opt=opt, net=net)
            trainer.fit()
        elif 'Restore_RWKV' == opt.network :
            print('load trainer : ' + 'trainer_Restore_RWKV')
            trainer = load_class('trainers', 'trainer_Restore_RWKV', opt=opt, net=net)
            trainer.fit()
        else:
            print('load trainer : ' + 'piglet_trainer')
            trainer = load_class('trainers',  'piglet_trainer', opt=opt, net=net)
            trainer.fit()
    elif opt.trainer_mode == 'test':
        if 'PIPNet_Restormer_onskip_inter' == opt.network :
            tester = load_class('trainers', 'tester_PIP', opt=opt, net=net)
            tester.run()
        else:
            tester = load_class('trainers', 'piglet_tester', opt=opt,net=net)
            tester.run()
    print('finish')
