...
class DuDotrainer(BasicTrainer):
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            ...
            final_img_mu, sino_output_mu = self.net(ma_sinogram, ma_CT_norm, mask_sinogram)
            loss = self.criterion(final_img_mu, gt_CT_mu) + self.criterion(sino_output_mu, gt_sinogram)
            ...
            restored = self.transformData.normalize(self.cttool.mu2HU(final_img_mu))
            restored = torch.cat([restored,restored,restored], dim=1)
