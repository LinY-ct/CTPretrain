    ...
    def run(self, ):
        ...
        with torch.no_grad():
            for i, data in enumerate(pbar):
                ...
                restored_mu, sino_mu = self.net(ma_sinogram,ma_CT_norm,mask_sinogram)
                restored_hu = self.cttool.mu2HU(restored_mu)
                restored = self.transformData.normalize(restored_hu)
                # import matplotlib.pyplot as plt
                # plt.imshow(gt_CT_hu.cpu().numpy()[0, 0, :, :], cmap='gray')
                # plt.title('GT')
                # plt.show()
                ...
                rmse, psnr, ssim = compute_measure(restored, gt_CT_norm[:,0:1,:,:], data_range=1)
