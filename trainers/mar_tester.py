...
from datasets.MAR import MARDataset


class mar_tester(BasicTrainer):
    ...
    def prepare_dataset(self, ):
        ...
        if dataset_name == 'mar':
            self.test_dataset = MARDataset(opt.dataset_path, mode='test', 
                                            dataset_shape=opt.dataset_shape,use_num=opt.num_train)
            
        else:
            raise NotImplementedError(f'Dataset {dataset_name} not implemented, try mar.')
    ...
    def run(self, ):
        ...
        with torch.no_grad():
            for i, data in enumerate(pbar):
                gt_CT_mu, ma_CT_mu, filename = data
                gt_CT_mu = gt_CT_mu.to('cuda')
                ma_CT_mu = ma_CT_mu.to('cuda')
                ...
                restored  , _= self.net(ma_CT_norm)
    ...
    def processingImg(self, image_data):
        truncated_min = -200
        truncated_max = 300

        image_data[image_data < truncated_min] = truncated_min
        image_data[image_data > truncated_max] = truncated_max
        image_data_normalized = (image_data - truncated_min) / (truncated_max - truncated_min)
        img = Image.fromarray((image_data_normalized * 255).astype(np.uint8))
        return img

    def save_png(self, **mdict):
        data_save = {}
        for key, value in mdict.items():
            if key not in ['filename', 'de_value', 'de_type']:
                data_save[key] = value.cpu().squeeze_().data.numpy()
                ...
                data_save[key] = value
        data_name = "".join(mdict['filename']).split('.')[0]        
        os.makedirs(self.save_dir + '/GT', exist_ok=True)
        os.makedirs(self.save_dir + '/Input', exist_ok=True)
        os.makedirs(self.save_dir + '/Output', exist_ok=True)

        img = self.processingImg(data_save['gt_ct'])
        img.save(os.path.join(self.save_dir, 'GT',data_name + '_gt.png'))

        img = self.processingImg(data_save['de_ct'])
        img.save(os.path.join(self.save_dir, 'Input',data_name + '_input.png'))

        img = self.processingImg(data_save['output'])
        img.save(os.path.join(self.save_dir, 'Output',data_name + '_output.png'))
