...
class piglet_trainer_GGB3CL_v2(BasicTrainer):
    def __init__(self, opt, net):
        ...
        if dataset_name == 'aapm':
            ...
            self.val_dataset = DeeplessionDataset(opt.dataset_path, mode='val',
                                              dataset_shape=opt.dataset_shape,
                                              use_num=opt.num_val)
        elif dataset_name == 'piglet':
            self.train_dataset = PigletDataset(opt.dataset_path, mode='train',
                                               dataset_shape=opt.dataset_shape,
                                               use_num=opt.num_train,returnType=True)
            self.val_dataset = PigletDataset(opt.dataset_path, mode='val',
                                             dataset_shape=opt.dataset_shape,
                                             use_num=opt.num_val)
        else:
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            gt_CT_hu, ma_CT_hu, filename, ma_CT_hu1,ma_CT_hu2,ma_CT_hu3 = data
            gt_CT_hu = torch.cat([gt_CT_hu, gt_CT_hu, gt_CT_hu], dim=1).cuda()
            ma_CT_hu = torch.cat([ma_CT_hu, ma_CT_hu, ma_CT_hu], dim=1).cuda()
            ma_CT_norm = self.transformData.normalize(ma_CT_hu)
            gt_CT_norm = self.transformData.normalize(gt_CT_hu)


            # negative generate
            b, c, h, w = gt_CT_hu.size()
            negative_CT_hu = torch.zeros((b, 3, 3, w, h))
            for j in [ma_CT_hu1,ma_CT_hu2,ma_CT_hu3]:
                k = 0
                neg = torch.cat([j,j,j], 1)
                negative_CT_hu[0:, k, :, :, :] = neg[0:, :, :, :]
                k += 1
            negative_CT_norm = self.transformData.normalize(negative_CT_hu).cuda()

            restored, mask  = self.net(ma_CT_norm)
            loss = 0.6 * self.criterion(restored, gt_CT_norm) + 0.4 * self.CLLoss(mask, ma_CT_norm,
                                                                                gt_CT_norm,
                                                                                negative_CT_norm,
                                                                                restored)
    ...
    def val(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(pbar):
                gt_CT_hu, ma_CT_hu, filename = data
