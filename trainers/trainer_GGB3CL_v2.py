...
class trainer_GGB3CL_v2(BasicTrainer):
    def __init__(self, opt, net):
        ...
        if dataset_name == 'aapm':
            ...
            self.train_dataset = DeeplessionDataset(opt.dataset_path,mode='train',
                                                    ...
                                                    de_type=opt.de_type,dose=opt.dose,view=opt.view)
            self.val_dataset = DeeplessionDataset(opt.dataset_path, mode='val',
                                              dataset_shape=opt.dataset_shape,
                                              use_num=opt.num_val,de_type=opt.de_type,dose=opt.dose,view=opt.view)
        else:
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            ...
            negative_CT_mu = torch.zeros((b, 4, 3, w, h))
            for j in ("ld", 'sv', 'lv','ld_sv', 'ld_lv'):
                k = 0
                if (j == degenetaion_type[0].split("_")[-1]):
                    ...
                    k += 1
            # print("="*20)
            # print("negative_CT_mu.shape",negative_CT_mu.shape)
            # print("="*20)
