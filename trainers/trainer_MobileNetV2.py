...
class trainer_MobileNetV2(BasicTrainer):
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            ...
            de_dict = {"ld": 0, "sv": 1, 'lv': 2, 'ld_sv': 3, 'ld_lv': 4}
            de_id = torch.tensor(de_dict[degenetaion_type[0]]).view(1).cuda()
            # print('de_id:', de_id)
            restored   = self.net(ma_CT_norm)
            loss = self.criterion(restored, de_id)
            ...
            if self.opt.use_tqdm:
                pbar.set_postfix(
                    {'epoch': '%d' % (self.epoch), 'loss': '%.6f' % (loss.item()), 'acc': '%.2f' % (rmse),
                    })
                pbar.update(1)
            ...
            if self.iter != 0 and self.iter % self.itlog_intv == 0:
                log_info = {
                    ...
                    'acc': np.mean(rmses[-self.itlog_intv:]),
        ...
        print(f'Epoch {self.epoch} train acc: {epoch_log["acc"]}')
    ...
    def val(self):
        ...
        print(f'Epoch {self.epoch} val acc: {epoch_log["acc"]}')
