import sys
sys.path.append('..')
sys.path.append('../models/facebookresearch_dinov2_main')
import os
import torch
...
from utilities.metrics import compute_RMSE
...
class trainer_dinov2(BasicTrainer):
    def __init__(self, opt, net):
        ...
        self.dinov2_model = dinov2_vitb14(pretrained=True).cuda().eval()
        #torch.hub.load('facebookresearch/dinov2', 'dinov2_vitb14',pretrained=True).cuda().eval()
        #torch.hub.load('/home/wsy/.cache/torch/hub/facebookresearch_dinov2_main',
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            ...
            gt_CT_norm = self.transformData.normalize(gt_CT_hu).cuda()


            restored   = self.net(ma_CT_norm)

            gt_CT_norm = self.dinov2_model.forward_features(F.interpolate(gt_CT_norm, (224, 224), mode='bilinear'))['x_norm_patchtokens']
            ...
            rmse = compute_RMSE(restored, gt_CT_norm)
            rmses.append(rmse)
            # psnrs.append(psnr)
            # ssims.append(ssim)
            if self.opt.use_tqdm:
                pbar.set_postfix(
                    {'epoch': '%d' % (self.epoch), 'loss': '%.6f' % (loss.item()), 'rmse': '%.4f' % (rmse),
                     })
                pbar.update(1)
