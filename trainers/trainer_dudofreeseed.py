...
class trainer_dudofreeseed(BasicTrainer):
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            ...
            gt_CT_hu = self.cttool.mu2HU(gt_CT_mu)
            ...
            output_mu, _, output_sino_img = self.net(ma_sinogram, ma_CT_mu, mask_sinogram, only_train_img_net=False)
            loss = self.criterion(output_mu, gt_CT_mu) + self.criterion(output_sino_img, gt_CT_mu)
            self.optimizer.zero_grad()
            loss.backward()
            self.optimizer.step()

            losses.append(loss.item())
            restored = self.cttool.mu2HU(output_mu)
            restored = self.transformData.normalize(restored)
