...
class trainer_freeseed(BasicTrainer):
    def __init__(self, opt, net1, net2):
        super().__init__()
        self.net1 = net1
        self.net2 = net2
        self.opt = opt
        ...
        if dataset_name == 'aapm':
            self.train_dataset = AAPMMyoDataset(opt.dataset_path, mode='train',
                                                ...
                                                use_num=opt.num_train,de_type=opt.de_type,
                                               dose=opt.dose,
                                               view=opt.view)
            self.val_dataset = AAPMMyoDataset(opt.dataset_path, mode='val',
                                                  dataset_shape=opt.dataset_shape,
                                                  use_num=opt.num_val,de_type=opt.de_type,
                                               dose=opt.dose,
                                               view=opt.view)
    ...
    def fit(self):
        ...
        # 统计模型参数量
        total_params = sum(p.numel() for p in self.net1.parameters() if p.requires_grad) / 1000
        total_params += sum(p.numel() for p in self.net2.parameters() if p.requires_grad) / 1000
        print("Total Parameters (K):", total_params)

        # init and resume optimizer
        self.optimizer1 = torch.optim.Adam(filter(lambda p: p.requires_grad, self.net1.parameters()),
                                          lr=self.opt.lr)
        self.optimizer2 = torch.optim.Adam(filter(lambda p: p.requires_grad, self.net2.parameters()),
                                          lr=self.opt.lr)
        self.scheduler1 = self.get_scheduler(self.optimizer1)
        ...
        for self.epoch in range(start_epoch, self.opt.epochs):
            ...
            if train_best_loss > train_current_loss:
                self.save_model(net=self.net1, net_name='best_train_free', loss=train_best_loss)
                self.save_opt(optimizer=self.optimizer1, scheduler= self.scheduler1, opt_name='best_train_free')
                self.save_model(net=self.net2, net_name='best_train_seed', loss=train_best_loss)
                self.save_opt(optimizer=self.optimizer2, scheduler= self.scheduler2, opt_name='best_train_seed')
                train_best_loss = train_current_loss
            if val_best_loss > val_current_loss:
                self.save_model(net=self.net1, net_name='best_val_free', loss=train_best_loss)
                self.save_opt(optimizer=self.optimizer1, scheduler= self.scheduler1, opt_name='best_val_free')
                self.save_model(net=self.net2, net_name='best_val_seed', loss=train_best_loss)
                self.save_opt(optimizer=self.optimizer2, scheduler= self.scheduler2, opt_name='best_val_seed')
                val_best_loss = val_current_loss
            if ((self.epoch + 1) % self.opt.save_epochs == 0) or ((self.epoch + 1) == self.opt.epochs):
                self.save_model(net=self.net1, net_name='current_free', loss=train_best_loss)
                self.save_opt(optimizer=self.optimizer1, scheduler= self.scheduler1, opt_name='current_free')
                self.save_model(net=self.net2, net_name='current_seed', loss=train_best_loss)
                self.save_opt(optimizer=self.optimizer2, scheduler= self.scheduler2, opt_name='current_seed')
    @staticmethod
    ...
    def train(self):
        ...
        for i, data in enumerate(pbar):
            ...
            gt_CT_mu, ma_CT_mu, gt_sinogram, ma_sinogram, mask_sinogram = \
                self.net1.make_traindata2(gt_CT_mu, gt_CT_mu, degenetaion_type[0], dose_range[0].item(), view[0].item())
            ...
            neg_art, final_mu = self.net1(ma_CT_norm)
            self.set_requires_grad(self.net2, True)
            proxy_mu = self.net2(final_mu.detach())
            loss2 = self.criterion(proxy_mu, gt_CT_norm)
            ...
            loss = self.criterion(final_mu, gt_CT_norm) + self.criterion(proxy_mu * mask, gt_CT_norm * mask) * 0.5
            ...
            rmse, psnr, ssim = compute_measure(final_mu, gt_CT_norm,data_range=1)
            rmses.append(rmse)
            psnrs.append(psnr)
            ssims.append(ssim)
        ...
        epoch_log = {
            ...
            'ssim': np.mean(ssims),
            'psnr': np.mean(psnrs),}
        current_lr = self.optimizer1.state_dict()['param_groups'][0]['lr']
    ...
    def val(self):
        ...
        with torch.no_grad():
            for i, data in enumerate(pbar):
                ...
                neg_art, final_mu = self.net1(ma_CT_norm)
                proxy_mu = self.net2(final_mu)
                loss = self.criterion(final_mu, gt_CT_norm) + self.criterion(proxy_mu, gt_CT_norm)
